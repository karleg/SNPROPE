...
for (sim.itr in 1:10)
{
  ...
  system('sh snptest_bayesian.sh')
  
  snptest.out=read.table('snptest_out',header = T)
  
  bf=snptest.out$bayesian_add_log10_bf
  
  if (max(abs(bf),na.rm = T)<log10(2))
  {
    tp=c(tp,0)
  ...
  tp=c(tp,sum(unique(snptest.out$rsid[abs(bf)>log10(2) & !is.na(bf)]) %in% paste0('snp',1:num.mutations))/sum(abs(bf)>log10(2),na.rm=T)) #TP
  
  fp=c(fp,sum(!unique(snptest.out$rsid[abs(bf)>log10(2) & !is.na(bf)]) %in% paste0('snp',1:num.mutations))/sum(abs(bf)>log10(2),na.rm=T)) #FP
  
  recall=c(recall,sum(unique(snptest.out$rsid[abs(bf)>log10(2) & !is.na(bf)]) %in% paste0('snp',1:num.mutations))/num.mutations) #Recall
}
...
write.table(out.file,'sim_snptest_bayesian.txt',sep='\t',row.names = F,col.names = T,quote = F)
